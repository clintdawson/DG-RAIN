...
CONTAINS
  ...
  FUNCTION evalf2 (i, Val) RESULT (res)
    ...
    DO IP=1,Comp(i)%ByteCodeSize
       ...
       CASE   (cDiv); IF (Comp(i)%Stack(SP)==0._sz) THEN; EvalErrType2=1; res=zero; RETURN; ENDIF
       ...
       CASE (cLog10); IF (Comp(i)%Stack(SP)<=0._sz) THEN; EvalErrType2=3; res=zero; RETURN; ENDIF
                      Comp(i)%Stack(SP)=LOG10(Comp(i)%Stack(SP))
       CASE   (cLog); IF (Comp(i)%Stack(SP)<=0._sz) THEN; EvalErrType2=3; res=zero; RETURN; ENDIF
                      Comp(i)%Stack(SP)=LOG(Comp(i)%Stack(SP))
       CASE  (cSqrt); IF (Comp(i)%Stack(SP)<0._sz) THEN; EvalErrType2=3; res=zero; RETURN; ENDIF
       ...
       CASE  (cAsin); IF ((Comp(i)%Stack(SP)<-1._sz).OR.(Comp(i)%Stack(SP)>1._sz)) THEN
                      EvalErrType2=4; res=zero; RETURN; ENDIF
